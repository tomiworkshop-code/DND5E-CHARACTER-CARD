...
// Update UI
const targetClassesUI = `                <div v-if="activeModule === 'classes'" class="space-y-4">
...
html = html.replace(targetClassesUI, newClassesUI);


const targetClassesView = `<div v-for="(cls, idx) in selectedChar.classes" :key="idx" class="px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg flex justify-between items-center text-sm">
                        <span class="font-bold text-gray-800">{{ cls.name || '未知職業' }}</span>
                        <span class="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-100">Lv {{ cls.level || 0 }}</span>
                      </div>`;

const newClassesView = `<div v-for="(cls, idx) in selectedChar.classes" :key="idx" class="px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg flex flex-col gap-1 text-sm">
                        <div class="flex justify-between items-center">
                          <span class="font-bold text-gray-800">{{ getClassName(cls) || '未知職業' }}</span>
                          <span class="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-100">Lv {{ cls.level || 0 }}</span>
                        </div>
                        <div v-if="cls.subclass" class="text-xs text-gray-500">{{ getSubclassName(cls) }}</div>
                      </div>`;
html = html.replace(targetClassesView, newClassesView);

// Inject subclass logic into setup
const targetSetupRet = `isDMWorld,
          requestDMApproval
        };`;

const newSetupRet = `isDMWorld,
          requestDMApproval,
          subclassDef: (cl) => {
            const def = CLASSES.find(k => k.name_en === (cl.name_en || cl.name));
...
html = html.replace(targetSetupRet, newSetupRet);
