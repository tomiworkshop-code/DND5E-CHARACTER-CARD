...
### 3.3 提案 → 定案流程（§3 核心）— ✅ 完成 (DM v2.4.0 / Build 0721.8；玩家版 v2.2.5)
- [x] 玩家快照 = 提案；roster 卡/抽尜標示「📝未定案」/「✅已定案 (v_m N)」。
- [x] 「📥 採納為世界存檔」：將玩家提案寫入 DM 正史 `world.charSaves[cid]`（dmv2 隱離，不回送）。
- [x] 「✏️ 就地編輯定案」：DM 改 HP(cur/max/temp)/AC → `version_m` 前進 → 只送「最小 partial save」
      `setSave(db,roomId,cid,{hp,ac,_sync})` 回送 → 玩家端 handleRemoteSave/mergeInstance 採用。
- [x] 【安全】partial 只含 DM 改動欄位；applyZone 只套存在 key → 玩家技能/背包/魔寵/敘事完整保留（非摘要覆蓋）。
- [x] 玩家端快照加送 `full.sync{version_m,n}` + `hp.temp`（DM 判 version_m 基準、不掉 temp）。
- [x] 未定案前不影響世界正史（charSaves 獨立於實體/備份）。
- 測試：`test_dmv2_step3_finalize.js`(19，真 schema 合流不損壞) + `test_dmv2_step3_ui.js`(44)；
      玩家版回歸全綠（p2b_onsave 49 / skills_saves 24 / familiars 2 / jsdom / migration）。
