...
- **② 團（Campaign Run / 房間）— 錨定在某個紀元節點**
  - 開團 meta 記 `worldId + questId? + eraId`（停在樹上哪個節點）；出團過程推進 / 長出新節點。
  - 不同團玩同一世界 → 自然停在不同節點/分支。
  - **開團綁任務（Tommy 2026-07-21 定案）**：DM 開團主流程 = 「選一個任務(quest)來跑」，
    `worldId` 由任務自動帶出（不必手選世界）。但要記得：
    - `questId` **可選**：允許不綁任務的自由團 / session zero。
    - `eraId` **獨立一軸**，推不出來：同一任務可在不同 era 節點/平行分支跑（本節哲學）。
      預設 = 該世界 `currentEraId`（canon tip），DM 可覆寫錨到別的節點。
    - 三軸釐清：world=哪棵現實樹（quest 可推得）、quest=跑什麼內容、era=此刻世界長什麼樣（quest 推不得）。
