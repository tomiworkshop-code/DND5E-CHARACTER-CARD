...
## 10. 世界紀元 / 世界狀態（World State / Era，Tommy 2026-07-21 定案：B 精簡版）
> 命名：DM 端稱「世界紀元 / 世界狀態」，避免與玩家端 worldProgress（角色存檔進度）混淆。
> 核心哲學（Tommy）：**角色活在當下**；地點/NPC/勢力各自帶著自己的經歷與故事往前演進。
> 世界是「線性向前推進」，不是一堆可回放的快照。整個團的時間線「回溯」最容易攪亂（智識穿越問題），
> 因此本階段**不做**快照回放/分支；僅保留未來擴充空間。

### 10.1 資料模型（B 精簡版：線性紀元 + entity 自帶狀態）
- **世界（world）新增**：
  - `eras: [{ id, name, order, summary, date? }]`：有序「紀元/篇章」清單（如「第一章：邊境動亂」→「第二章：北城陷落後」）。
  - `currentEraId`：當前紀元指標（世界「現在」進行到哪）。
- **entity（NPC / 地點 / 勢力 …）自帶狀態**（活在當下，累積演進）：
  - `status`：如 `active`（存在）/ `destroyed`（已毀）/ `changed`（已質變）/ `hidden`（未登場）。
  - `story` / `notes`：這個 NPC/地點自己的經歷與變化紀錄（文字，隨劇情累加）。
  - 可選 `sinceEraId`：從哪個紀元開始變成此狀態（供閱讀，非回放機制）。
- **世界「現在」= 所有 entity 當前 status 的總和**（不需為每個紀元存整份快照）。

### 10.2 出團連動（Step 3）
- 出團（開房）meta 記錄 `worldId + currentEraId`：出團時「除了選世界，還帶當前紀元」。
- 通常直接沿用世界的 `currentEraId`；DM 可在開團前推進/新增紀元。
- 與 landing「出團記錄」串接：一次出團的結果 → DM 可據此推進世界（新增紀元 / 更新 entity 狀態）。線性向前。

### 10.3 明確不做（本階段）＋ 未來擴充（parked）
- **不做**：整個世界時間線的快照回放、回到「城破之前」、分支時間線。
- **未來選配（需另設計、注意可管理性）**：穿越 / 死亡回溯 / 平行世界。
  - 屆時才需要「世界快照/版本 or 分支」模型；設計上線性紀元不應擋住日後加分支的路
    （eras 用 id 而非純序號、entity 狀態變更可留紀錄，未來要分叉再擴充）。
  - 導入前務必先解「時間線管理/智識穿越易亂」的 UX 問題，避免資料與敘事混亂。

### 10.4 排期
- Step 2b：entity CRUD 時一併加 `status` + `story/notes` 欄位；世界設定內加「紀元管理」（新增/排序/設當前紀元）。
- Step 3：出團 meta 帶 currentEraId；出團記錄 → 推進紀元的入口。
